...
router = fastapi.APIRouter(prefix="/accounts", tag=["accounts"])
...
@router.get(
    path="/{id}",
    name="accountss:read-account-bi-id",
...
async def get_account(
    ...
    try:
        ...
        raise await http_400_exc_id_not_found_request(id=id)
...
async def update_account(
    ...
    account_repo: AccountCRUDRepository = fastpi.Depends(get_repository(repo_type=AccountCRUDRepository)),
) -> AccountInResponse:
    ...
    try:
        ...
        raise await http_404_exec_id_not_found_request(id=query_id)
